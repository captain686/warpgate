...
impl RemoteClient {
    ...
    async fn connect(&mut self, ssh_options: TargetSSHOptions) -> Result<(), ConnectionError> {
        ...
        let (session, mut event_rx) = if let Some(jump_host) = &ssh_options.jump_host {
            ...
            let (jump_event_tx, jump_event_rx) = channel(REMOTE_CLIENT_EVENT_QUEUE_CAPACITY);
            ...
            let (event_tx, event_rx) = channel(REMOTE_CLIENT_EVENT_QUEUE_CAPACITY);
    ...
    async fn wait_for_connection<Fut>(
        ...
        fut_connect: Fut,
        mut event_rx: Receiver<ClientHandlerEvent>,
        is_jump_host: bool,
    ) -> Result<(Handle<ClientHandler>, Receiver<ClientHandlerEvent>), ConnectionError>
    where
    ...
    async fn authenticate_session(
        ...
        match auth {
            ...
            SSHTargetAuth::PublicKey(_) => {
                let best_hash = abortable!(session.best_supported_rsa_hash())?.flatten();
                ...
                for key in keys {
                    ...
                    let mut response = abortable!(session.authenticate_publickey(
                        username.to_string(),
                    ...
                    {
                        response = abortable!(session.authenticate_publickey(
                            username.to_string(),
            ...
            SSHTargetAuth::IamRole(_) => {
                let instance_info = abortable!(warpgate_aws::find_instance_by_ip(host))?;
                ...
                let best_hash = abortable!(session.best_supported_rsa_hash())?.flatten();
                let response = abortable!(session.authenticate_publickey(
                    username.to_string(),
                    PrivateKeyWithHashAlg::new(key.clone(), best_hash),
                ))?;
